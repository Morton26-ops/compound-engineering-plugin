...
```
the compound-engineering-agent-native-architecture skill
```
...
Launch 8 parallel sub-agents using the use_subagent tool with `subagent_type: Explore`, one for each principle. Each agent should:
...
1. Check for these 7 discovery mechanisms:
   ...
   - Slash commands (/help, the tools skill)
