...
- First set the the model skill to Haiku
...
2. Start work on approved items:
   ...
   ```bash
   the resolve_todo_parallel skill  # Work on multiple approved items efficiently
   ```
...
- ❌ That's for the resolve_todo_parallel skill phase
```
...
1. run the resolve_todo_parallel skill to resolve the todos
2. commit the todos
3. nothing, go chill
```
