...
describe("writeCursorBundle", () => {
  ...
  test("rules use .mdc file extension", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cursor-ext-"))
    const bundle: CursorBundle = {
    ...
    expect(await exists(path.join(tempRoot, ".cursor", "rules", "test-rule.mdc"))).toBe(true)
    expect(await exists(path.join(tempRoot, ".cursor", "rules", "test-rule.md"))).toBe(false)
  })

  test("writes directly into .cursor output root without double-nesting", async () => {
  ...
  test("commands written as .md files in commands directory", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "cursor-cmd-"))
    const bundle: CursorBundle = {
      rules: [],
      commands: [
        {
    ...
    const cmdPath = path.join(tempRoot, ".cursor", "commands", "deploy.md")
