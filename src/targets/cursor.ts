...
export async function writeCursorBundle(outputRoot: string, bundle: CursorBundle): Promise<void> {
  ...
  if (bundle.rules.length > 0) {
    for (const rule of bundle.rules) {
      await writeText(path.join(paths.cursorDir, "rules", `${rule.name}.mdc`), rule.content + "\n")
    }
  }

  if (bundle.commands.length > 0) {
    for (const command of bundle.commands) {
      await writeText(path.join(paths.cursorDir, "commands", `${command.name}.md`), command.content + "\n")
    }
  }
